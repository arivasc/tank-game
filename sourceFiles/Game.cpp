...
{
	this->window = new sf::RenderWindow(sf::VideoMode(700, 700), "Swaglords of Space - Game 3", sf::Style::Close | sf::Style::Titlebar);
...
void Game::initTextures()
{
	...
	this->textures["ENEMY"]->loadFromFile("Textures/tank1-rotDer3.png");
}
...
void Game::initGUI()
{
	...
	this->menu = new Menu(this->window->getSize());
	this->kinematics1 = new Kinematics(this->window->getSize());

    this->tankEnemy = new TankEnemy(this->textures["ENEMY"],this->textures["BULLET"],550,10);
    this->buffer.loadFromFile("sound/click.wav");
    this->sonido.setBuffer(this->buffer); 
}
...
void Game::initObstacle(){
	...
	this->obstacles.push_back(new ObstacleB(61.f,55.f,1));
	this->obstacles.push_back(new ObstacleB(171.f,55.f,1));
	this->obstacles.push_back(new ObstacleB(281.f,55.f,1));
}
...
Game::~Game()
{
	...
	delete this->kinematics1;
	delete this->menu;
	delete this->tankEnemy;
	//Delete textures
...
void Game::run()
{
	...
	while (this->window->isOpen())
	{
		...
		// 		this->update();
		if(this->finMenu == 0){
			this->menu->update(*this->window,&this->sonido);
		}else if(this->finKinematics1 == 0){
			this->kinematics1->update(this->clock);
		}else{
			if(this->player->getHp() > 0)
				this->update();
		}

		// if(this->finKinematics1 == 0){
		// 	this->kinematics1->render(*this->window,this->clock);
		// 	if(this->kinematics1->getWindow() == 2){
		// 		this->finKinematics1 = 1;
		// 	}
		// }else{
		// 	this->render();
		// }
...
void Game::updateCollisionObstacle(){
	...
	// 	if(this->player->getBoundsCircle().intersects(ob->getBounds())){
	// 		this->player->setPosition(*this->previousPosition);
	// 	}else{
	// 		*this->previousPosition = this->player->getPosCircle();
	// 	}
	// }
	if(this->player->getBoundsCircle().intersects(this->obstacles[0]->getBounds()) 
	  || this->player->getBoundsCircle().intersects(this->obstacles[1]->getBounds())
	  || this->player->getBoundsCircle().intersects(this->obstacles[2]->getBounds())){
		this->player->setPosition(*this->previousPosition);
	}else{
		*this->previousPosition = this->player->getPosCircle();
	}
...
void Game::update()
{
	...
	this->tankEnemy->update(this->player->getSprite());
	this->updateGUI();
...
void Game::render()
{
	...
	if(this->finMenu == 0){
		...
		if(this->menu->getWindow() == 1){
			this->finMenu = 1;
			delete this->menu;
			this->clock->restart();
			this->buffer.loadFromFile("sound/cinematica1.wav");
    		this->sonido.setBuffer(this->buffer);
			this->sonido.play(); 
		}
	}else if(this->finKinematics1 == 0){
		this->kinematics1->render(*this->window,this->clock);
		if(this->kinematics1->getWindow() == 3){
			this->finKinematics1 = 1;
			delete kinematics1;
			this->sonido.stop();
		}
		...
		this->tankEnemy->render(this->window);
